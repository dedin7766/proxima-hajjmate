...
export default function CartTable({ items, onUpdateQty, onUpdateDiscount, onRemove }: CartTableProps) {
    return (
        <div className="flex-grow overflow-y-auto -mx-4 sm:-mx-6 px-4 sm:px-6 border-t border-b">
            <table className="table-auto w-full">
                ...
                <tbody>
                    {items.length === 0 ? (
                        <tr><td colSpan={6} className="text-center p-8 text-muted-foreground">Keranjang masih kosong</td></tr>
                    ) : (
                        items.map((item) => (
                            <tr key={item.product_id} className="border-b">
                                ...
                                <td className="p-3 text-right">
                                    <input type="number" min={0} placeholder='0' value={item.discount === 0 ? '' : item.discount} className="w-24 text-right border bg-background rounded-md py-1 px-2" onChange={(e) => onUpdateDiscount(item.product_id, Number(e.target.value))} onFocus={(e) => e.target.select()} />
                                </td>
                                <td className="p-3 text-right font-medium">Rp {item.subtotal.toLocaleString()}</td>
                                <td className="p-3 text-center">
                                    <button onClick={() => onRemove(item.product_id)} className="text-destructive hover:text-destructive/80 p-2"><X size={20} /></button>
                                </td>
                            </tr>
                        ))
                    )}
